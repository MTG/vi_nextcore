...
class ConvBaselineJP5LINMP(nn.Module):
    """
    ...
    def __init__(self, bn=0, lin1=256, lin2=0, sum_method=0, autopool_p=0, cla=0, use_sigmoid=1, mulprelu=0, mulautop=0):
        ...
        self.prelu5 = nn.PReLU(num_parameters=512, init=0.01)
    ...
    def forward(self, data, mask_idx):
        ...
        x = self.prelu2(self.in2(self.conv2(x)))
        x = F.max_pool2d(x, kernel_size=(1, 2))
        x = self.prelu3(self.in3(self.conv3(x)))
        x = F.max_pool2d(x, kernel_size=(1, 2))
        x = self.prelu4(self.in4(self.conv4(x)))
        x = F.max_pool2d(x, kernel_size=(1, 2))
        x = self.prelu5(self.in5(self.conv5(x)))
