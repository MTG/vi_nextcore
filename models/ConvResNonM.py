...
class ConvResNonM(nn.Module):
    ...
    def __init__(self, sum_method=0, autopool_p=0, cla=0):
        """
            Initializing the network
        """
        super().__init__()

        self.input_conv1 = nn.Conv2d(in_channels=1,
                                    out_channels=128,
                                    kernel_size=(5, 5),
                                    bias=False)
        nn.init.kaiming_normal_(self.input_conv1.weight, a=0.01, mode='fan_out', nonlinearity='leaky_relu')
        self.prelu1_1 = nn.PReLU(init=0.01)
        self.bn1_1 = nn.BatchNorm2d(128)
        nn.init.constant_(self.bn1_1.weight, 1)
        nn.init.constant_(self.bn1_1.bias, 0)

        self.input_conv2 = nn.Conv2d(in_channels=128,
                                     out_channels=128,
                                     kernel_size=(5, 5),
                                     bias=False)
        nn.init.kaiming_normal_(self.input_conv2.weight, a=0.01, mode='fan_out', nonlinearity='leaky_relu')
        self.prelu1_2 = nn.PReLU(init=0.01)
        self.bn1_2 = nn.BatchNorm2d(128)
        nn.init.constant_(self.bn1_2.weight, 1)
        nn.init.constant_(self.bn1_2.bias, 0)

        self.input_conv3 = nn.Conv2d(in_channels=128,
                                     out_channels=256,
                                     kernel_size=(4, 4),
                                     bias=False)
        nn.init.kaiming_normal_(self.input_conv3.weight, a=0.01, mode='fan_out', nonlinearity='leaky_relu')
        self.prelu1_3 = nn.PReLU(init=0.01)
        self.bn1_3 = nn.BatchNorm2d(256)
        nn.init.constant_(self.bn1_3.weight, 1)
        nn.init.constant_(self.bn1_3.bias, 0)
    ...
    def forward(self, data, mask_idx):
        ...
        x = self.prelu1_1(self.bn1_1(self.input_conv1(data)))

        x = self.prelu1_2(self.bn1_2(self.input_conv2(x)))

        x = self.prelu1_3(self.bn1_3(self.input_conv3(x)))
