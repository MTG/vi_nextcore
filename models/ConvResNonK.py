...
class ConvResNonK(nn.Module):
    ...
    def __init__(self, cla=0):
        ...
        self.block1 = BasicBlock(init=128)
        self.block2 = BasicBlock(init=128)
        self.block3 = BasicBlock(init=128)

        self.conv_dil1 = nn.Conv2d(in_channels=128,
                               out_channels=256,
                               kernel_size=(1, 5),
                               padding=(0, 10),
                               dilation=(1, 5),
    ...
    def forward(self, data, mask_idx):
        ...
        x = torch.sum(x[:, :128] * F.softmax(x[:, 128:], dim=2), dim=2, keepdim=True)
...
class BasicBlock(nn.Module):
    def __init__(self, init=256):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=init,
                               out_channels=init,
                               kernel_size=(1, 5),
                               padding=(0, int(5/2)),
                               bias=False)
        ...
        self.conv2 = nn.Conv2d(in_channels=init,
                               out_channels=init,
                               kernel_size=(1, 5),
                               padding=(0, int(5/2)),
                               bias=False)
