...
class PairsAugFixedSize(Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        if item1.shape[2] >= self.w:
            p_index = [i for i in range(0, item1.shape[2] - self.w + 1)]
            if len(p_index) != 0:
                start = np.random.choice(p_index)
                item1 = item1[:, :, start:start+self.w]
        else:
            item1 = torch.cat((item1, torch.zeros([1, self.h, self.w - item1.shape[2]])), 2)
